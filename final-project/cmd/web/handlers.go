...
func (app *Config) ActivateAccount(w http.ResponseWriter, r *http.Request) {
	...
	app.Session.Put(r.Context(), "flash", "Account activated. You can log in now.")
	http.Redirect(w, r, "/", http.StatusSeeOther)
}
...
func (app *Config) generateManual(user data.User, plan *data.Plan) *gofpdf.Fpdf {
	...
	pdf.MultiCell(0, 4, fmt.Sprintf("Name: %s %s", user.FirstName, user.LastName), "", "C", false)
	pdf.Ln(5)
	pdf.MultiCell(0, 4, fmt.Sprintf("%s User Guid", plan.PlanName), "", "C", false)
