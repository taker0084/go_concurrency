...
var pageTests = []struct {
	...
	},
	{
		...
		sessionData:        map[string]any{"userID": 1},
	},
}
...
func Test_Pages(t *testing.T) {
	...
	for _, e := range pageTests {
		...
		if rr.Code != e.expectedStatusCode {
			t.Errorf("%s failed: %d", e.name, rr.Code)
		}
...
func TestConfig_PostLoginPage(t *testing.T) {
	...
	if rr.Code != http.StatusSeeOther {
		t.Errorf("PostLoginPage failed: %d", rr.Code)
	}

	if !testApp.Session.Exists(ctx, "userID") {
		t.Error("PostLoginPage failed: session does not exist")
	}
}

func TestConfig_SubscribeToPlan(t *testing.T) {
	rr := httptest.NewRecorder()
	req, _ := http.NewRequest("GET", "/member/subscribe?id=1", nil)
	ctx := getCtx(req)
	req = req.WithContext(ctx)
	...
	if !testApp.Session.Exists(ctx, "userID") {
		t.Error("SubscribeToPlan failed: session does not exist")
	}
}
